#pragma once
/*=============================================================
 * ui_state.h  –  UI layer state and asset handles
 *
 * Defines the UIState struct that bridges the raw game
 * simulation (GameState) with the renderer and input layers.
 * Owns camera state, selection bookkeeping, textures/models,
 * and audio handles.
 *=============================================================*/
#include "game.h"
#include "raylib.h"
...
typedef struct {
    ...
    bool          attack_move_mode;     /* next click is an attack-move */
    float         move_marker_tx;
    float         move_marker_ty;
    float         move_marker_start;

    /* Multiplayer lobby */
    char net_ip[64];                    /* IP address the client connects to */
    ...
    /* ── 3D models (hero possession view) ──────────────────── */
    ...
    int       osm_tiles_loaded;         /* 0 = not loaded */
} UIState;
...
/* Returns a scale factor relative to 720p so HUD elements are
   legible on any screen size, including high-DPI phones.
   Range: [1.0, 2.5]. */
float hud_scale(void);

/* ── Top-level rendering & input entry points ──────────────── */
